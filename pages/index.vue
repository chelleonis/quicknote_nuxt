<template>
  <div>
    ...
    <div>
      ...
      <div>
        <button @click="accessLevel = 'public'">Public</button>
        <button @click="accessLevel = 'private'">Private</button>
      </div>
      <p>
        {{
          accessLevel === "public"
      ...
      <input v-model="docTitle" id="docTitle" type="text" required />
    </div>
    <div>
      <button @click="createDocument" :disabled="loading">
    ...
    <div v-if="documentInfo">
      ...
      <div v-if="documentInfo.accessKey">
        ...
        <p>Save this access key! you'll need it to access the document!</p>
        <code>
          {{ documentInfo.accessKey }}
        </code>
        <button @click="copyAccessKey">Copy</button>
      </div>
      <NuxtLink :to="`/note/${documentInfo.documentId}`">
        Open Document →
      </NuxtLink>
    </div>
  </div>
...
async function createDocument() {
  ...
  try {
    const response = await $fetch("http://localhost:8802/document/new", {
      method: "POST",
      headers: {
